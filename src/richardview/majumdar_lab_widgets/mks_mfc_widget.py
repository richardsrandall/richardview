...
class MksMFCWidget(generic_widget.GenericWidget):
    ...
    for what scale factor to use for a particular gas.
    ...
    :param default_scale_factor: A float representing the scale factor for the device. The default is 1. See the MKS manual to select factors for different gases.
    :type default_scale_factor: float, optional
    """
    ...
    def __init__(self,parent_dashboard,name,nickname,channel,**kwargs):
        ...
        default_scale_factor = kwargs['default_scale_factor'] if ('default_scale_factor' in kwargs.keys()) else 1
    ...
    def on_serial_query(self):
        ...
        for query in queries:
            write_me = "@"+str(self.device_id)+query+str(self.which_channel)+"?;FF"
    ...
    def on_serial_read(self):
        ...
        try:
            ...
            scale_factor = self.get_serial_object().readline().decode('ascii').replace(';FF','')
            ...
            # Parse the Mode data
            mode_status = self.get_serial_object().readline().decode('ascii').replace(';FF','')
            ...
            setpoint_status = self.get_serial_object().readline().decode('ascii').replace(';FF','')
            ...
            # Parse the flow data
            flow_status = self.get_serial_object().readline().decode('ascii').replace(';FF','')
    ...
    def construct_serial_emulator(self):
        ...
        :rtype: richardview.majumdar_lab_widgets.mks_mfc_widget.MksMFCSerialEmulator"""
...
class MksMFCSerialEmulator(generic_serial_emulator.GenericSerialEmulator):
    ...
    def write(self,value):
        ...
        # Emulator treats all devices as channel A1 on the vacuum controller
        value = value.decode('ascii')
        value=value.replace('2?;','1?;')
        value=value.replace('B1?;','A1?;')
        value = value.encode('ascii')
        # Give the appropriate response
        queries = (b'@001QMDA1?;FF',b'@001FRA1?;FF',b'@001QSPA1?;FF',b'@001QSFA1?;FF')
        if value not in queries:
            self.output_buffer.append(b'@001ACKA1whatever;FF')#A nonsense reply
            return
